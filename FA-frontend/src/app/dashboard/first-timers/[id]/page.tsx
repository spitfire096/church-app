...
export default function FirstTimerDetailPage() {
  ...
  const loadFirstTimer = async () => {
    try {
      setIsLoading(true);
      const data = await api.firstTimers.get(Number(params.id));
      setFirstTimer(data);
    } catch (err) {
      setError('Failed to load first timer details');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
    ...
    return (
      <DashboardLayout>
        <div className="bg-red-50 p-4 rounded-md">
          <p className="text-red-800">{error || 'First timer not found'}</p>
          <button 
            onClick={() => router.back()}
